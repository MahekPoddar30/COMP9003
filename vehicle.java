...
public class Main {
    public static void main(String[] args) {
        ...
        // Simulating movements for 5 iterations
        for (int i = 0; i < 5; i++) {
...
abstract class Vehicle {
    ...
    private int lane; // Current lane (1, 2, or 3)
    ...
    public Vehicle(String vehicleID) {
        ...
        this.lane = 1; // Starting in lane 1
        this.currentPosition = 0; // Starting at position 0
    }

    // Method to calculate velocity in meters per second
    public int velocity() {
        return velocity / 5; // Converting velocity to m/s
    }

    // Method to change lane with error handling
    public void changeLane(int newLane) throws SimulationException {
        ...
        this.lane = newLane; // Updating to new lane
    }
    ...
    public void move() {
        try {
            ...
            int newLane = (int) (Math.random() * 3) + 1; // Random lane between 1 and 3
            changeLane(newLane); // Attempt to change lane
        } catch (SimulationException e) {
            // Printing error message if lane change is invalid
...
// Car Class extending to Vehicle
...
// The truck Class extending Vehicle
class Truck extends Vehicle {
    ...
    @Override
    public void move() {
        // Specific behavior for Truck (if needed)
        super.move(); // Calling the move method from the Vehicle class
    }
}